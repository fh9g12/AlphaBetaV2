...
%         res(i).clean = jobs(j_i)<28;
        res(i).vanes = jobs(j_i)>33;
        res(i).U = mean(U);
        res(i).u = mean(V(1,:));
        res(i).v = mean(V(2,:));
        res(i).w = mean(V(3,:));
...
tt = tiledlayout(3,1);
for x_i = 1:length(Vs)
    tmp_res = farg.struct.filter(final_data,{{'U_inf',{'tol',Vs(x_i),2.5}},{'Radius',@(x)x<710}});
    nexttile(x_i)
...
for x_i = 1:length(Vs)
    tmp_res = farg.struct.filter(final_data,{{'U_inf',{'tol',Vs(x_i),2.5}}});
    nexttile(x_i)
