...
while true
    runLoop = logical(testscript_input_empty('Continue Testing? Choose 0 or 1',1));
    if runLoop == 0
        break;
    end
    ...
    isSteady = logical(testscript_input('Is Steady? Choose 0 or 1\n'));
    ...
    V_new = testscript_input(sprintf('\nPeak WRBM = %.2f Nm, Next WindSpeed?\n',peak_mom));
    if (V_new ~= 0) && abs(V_new - V_i)>5
        V_new = testscript_input(sprintf('Current V is %.2f m/s, Next WindSpeed?\n',V_i));
    end
    V_i = V_new;
